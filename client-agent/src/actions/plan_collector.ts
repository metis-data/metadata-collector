...
class PlanCollector {
  ...
  async fetchData({ dbConfig, client }: any) {
    const query = `
...
;`;
  ...
  shapeData({ dbConfig, data }: any) {
    try {
      return data.map((el: any) => makeSpan({ message: { ...el, plan: el?.plan }, ...dbConfig }));
    } catch (e) {
      this.logger.error('shapeData - error: ', e);
    }
  }

  async transferData({ payload, options }: any) {
    const { data = [] } = payload;
    const promises = [];
    for (let chuckedData of chuncker(data?.map((el: any) => JSON.stringify(el)))) {
