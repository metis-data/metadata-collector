...
function makeInternalHttpRequest(payload: any, options: any, numRetries = 0, ignoreStatusCodes = [], timeout = DEFAULT_REQUEST_TIMEOUT_IN_SEC) {
  ...
  return new Promise((resolve, reject) => {
    const req = provider.request(options, (res: any) => {
      let data = '';
      ...
      res.on('end', () => {
        ...
        if (res.statusCode < 400) {
          ...
          const error: any = new Error(`Problem with HTTPS request, status code is ${JSON.stringify(context, null, 2)
            }`);
