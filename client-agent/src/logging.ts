...
const getError = (msg: any, meta: any) => {
  if (isError(msg)) {
    return msg;
  }
...
const loggers: any = [];
const createSubLogger = (componentName: any, logLevel=LogLevelEnum.INFO) => { 
...
const logger: any = createSubLogger('app');


function consoleLog(level: any, stream: any, message: any) {
  if (stream) {
...
function log(level: any, stream: any, message: any) {
...
function loggerExit(msg: any) {
  ...
  loggers.forEach((loggerItem: any) => {
    loggerItem.close();
    loggerItem.end();
  });
}
